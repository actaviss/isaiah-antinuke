...
class EmbedCommands(commands.Cog):
    ...
    async def help(self, ctx):
       if category is None:
         ...
         embed.add_field(name=f"Usage!", value=f"`>help <CATEGORY>`", inline=False)
         ...
         embed.add_field(name=f"Usage!", value=f"`>[COMMAND]`", inline=False)
    ...
    async def setup(self, ctx):
        ...
        embed.add_field(name="Isaiah will be updated throughout time to ensure that your server is well protected!", value=f"To set up the antinuke, you must have isaiah **ABOVE** everyone, this makes sure that not even your staff can do anything risky.\n\nafter, whitelist your **TRUSTED** staff >`WHITELIST <@USER>`, only the guild owner is able to whitelist and unwhitelist! - by whitelisting a user, you allow them to have access to bypass anything that isaiah protects.\n\nif you ever wish to remove someone from the whitelist database, simple do >`UNWHITELIST <@USER>`\n\nto view the whitelisted users, do >`WHITELISTED`")
